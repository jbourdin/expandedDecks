...
class DevFixtures extends Fixture
{
    ...
    public function load(ObjectManager $manager): void
    {
        ...
        // Create archetypes
        $archetypeIronThorns = $this->createArchetype($manager, 'Iron Thorns ex', ['iron-thorns'], 'A powerful **Iron Thorns ex** deck built around the Paradox Pokemon. Uses heavy energy acceleration and spread damage to overwhelm opponents.', true, [PlaystyleTag::Aggressive, PlaystyleTag::Spread]);
        $archetypeAncientBox = $this->createArchetype($manager, 'Ancient Box', ['roaring-moon', 'flutter-mane'], 'The **Ancient Box** archetype combines multiple Ancient Pokemon to leverage Ancient support cards for a versatile attack strategy.', true, [PlaystyleTag::Toolbox, PlaystyleTag::Aggressive]);
...
MARKDOWN, true, [PlaystyleTag::Combo, PlaystyleTag::Lock, PlaystyleTag::Toolbox]);
        $archetypeKyurem = $this->createArchetype($manager, 'Kyurem', ['kyurem'], 'A **Kyurem** archetype that punishes Basic-heavy boards with its spread attack, often teched into [[archetype:regidrago]] builds.', true, [PlaystyleTag::Spread]);
        $archetypeSalamence = $this->createArchetype($manager, 'Salamence ex', ['salamence'], 'The **Salamence ex** archetype leverages its massive damage output to oneshot virtually anything. Commonly used as a finisher in [[archetype:regidrago]] lists.', true, [PlaystyleTag::Aggressive]);
        $archetypeLugia = $this->createArchetype($manager, 'Lugia Archeops', ['lugia', 'archeops'], null, false);
        $archetypeCharizardEevee = $this->createArchetype($manager, 'Charizard Flareon', ['charizard', 'eevee'], 'A **Charizard & Flareon** deck that evolves Eevee into Flareon for energy acceleration, fueling Charizard\'s massive fire attacks.', true, [PlaystyleTag::Aggressive, PlaystyleTag::Combo]);
