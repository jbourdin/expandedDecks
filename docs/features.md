...
| F3.11  | Event visibility                     | Medium   | Each event has a visibility setting: **public** (visible to all visitors, including unauthenticated users) or **attendees-only** (visible only to registered participants, event staff, and the organizer). Default: public. The event listing (F3.2) and detail view (F3.3) respect this setting. Organizers and admins always see all events regardless of visibility. |
| F3.12  | Event series                         | Low      | Group related events into a named **series** (e.g. "June Expanded Cup — Île-de-France"). A series has a name, optional description, date range, format, and organizer. Events can be linked to a series. A series has its own **public or attendees-only page** displaying the chronological list of its events with dates, locations (leagues), and results. Visibility follows the same rules as F3.11 — a public series page is accessible to all; an attendees-only series page is restricted to participants of at least one event in the series. |
