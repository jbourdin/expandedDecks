...
Minor release: the site becomes a **multilingual content platform**. A full translation workflow (epic [#612](https://github.com/jbourdin/expandedDecks/issues/612)) lets assigned translators localize CMS pages, archetypes, variant notes, menu categories, and banned/staple card texts through a moderated draft → review → publish pipeline, with revision history, staleness tracking, reader-facing notices, and notifications. Channels manage their locales entirely from the admin — including draft locales invisible to the public and brand-new languages added without a deploy.

### Features

- **Translation foundation (F9.7)** — A parameterless `#[Translatable]` attribute is the single registry of workflow fields. Per-type revision tables (`*TranslationRevision`) keep the full history of every locale **including the English source**, each translation revision pinned to the source revision it was based on; `TranslationRevisionListener` snapshots every live write inside the same flush and denormalizes `sourceOutdated` onto live rows via bulk SQL so public pages never query history. A backfill migration seeds revisions from all existing content. New `DeckTranslation` live table for archetype-variant notes (pattern bend: `Deck.notes` stays the canonical source; user decks never grow rows). ([#768](https://github.com/jbourdin/expandedDecks/pull/768))
- **Translation roles & voter (F9.8)** — `ROLE_TRANSLATION_EDITOR` paired with `User.translationLocales` (locales are data, not roles), distinct `ROLE_TRANSLATION_MODERATOR` inherited by CMS editors through the role hierarchy. `TranslationVoter` gates every workflow entry point; admins manage roles and locale assignments from the user admin page. ([#769](https://github.com/jbourdin/expandedDecks/pull/769))
- **Review workflow (F9.9)** — `translation_review` state machine (draft → pending_review → validated / rejected → draft). `TranslationReviewService` owns the transitions: submit refuses stale drafts (the source moved since), approve is the **single write path to live translation rows** (translator credited, `sourceOutdated` recomputed), reject carries a moderator comment. No self-approval shortcut, structurally. ([#770](https://github.com/jbourdin/expandedDecks/pull/770))
- **Translator workspace (F9.10–F9.13)** — `/admin/translations`: a queue (React) with per-content-type tabs and contributor/reviewer flavors, and a contextual split-view editor — English source pre-rendered with raw-markdown toggle, TipTap targets, SEO budget counters, pinned-vs-latest source **word diff** on stale drafts, guidelines panel. Public pages and archetype pages grow per-locale Translate entries in their action dropdown. The archetype view translates archetype + variants as one context (accordion, per-section revisions and actions); drafts can be **previewed on the public page** (`?translationPreview=1`, authorized users only). ([#771](https://github.com/jbourdin/expandedDecks/pull/771))
- **Reader-facing outdated notice (F9.14)** — Translated content whose English source moved shows a discreet notice linking to the source version, driven by the denormalized flag (zero extra queries). ([#771](https://github.com/jbourdin/expandedDecks/pull/771))
- **Workflow notifications (F9.15)** — Submissions notify moderators; approvals/rejections notify the contributor (with comment); a source edit notifies the credited translator of each fresh→outdated translation exactly once. In-app + email, preference-gated, rendered in the recipient's locale. Translator credit also joins the public content footer (« translated in … by … »). ([#771](https://github.com/jbourdin/expandedDecks/pull/771))
- **Draft channel locales (F9.16)** — A channel locale is published or **draft**: browsable only by its translators, moderators, and admins (marked entry in the switcher); everyone else is 302-redirected and no SEO surface (hreflang, sitemap, robots, og:locale) ever mentions it. Admin content forms cover draft locales so content is prepared before publication; publishing is one checkbox on the channel form. ([#777](https://github.com/jbourdin/expandedDecks/pull/777))
- **Banned & staple card translations (F9.17)** — Ban explanations and staple notes join the workflow (canonical English stays on the entity; card names are never translated). Listings render localized copy with fallback, the detail modal shows the outdated notice, drafts preview on the listing, and the queue gains card tabs with an untranslated-cards worklist for contributors. ([#778](https://github.com/jbourdin/expandedDecks/pull/778))
- **Admin-managed channel locales (F9.18)** — Any ISO 639-1 language can be added from the channel form (datalist + server validation), **always starting as a draft locale**; a warning lists locales lacking an XLIFF chrome file. Route locale requirements are permissive (`[a-z]{2}`) with `LocaleListener` gating unknown locales via 302; translator-assignable locales and profile language choices derive from channel data. ([#782](https://github.com/jbourdin/expandedDecks/pull/782))
- **Queue card tabs & locale filter (F9.19)** — Banned cards and staples get separate queue tabs; a locale segmented control appears when work spans several target languages. ([#783](https://github.com/jbourdin/expandedDecks/pull/783))

### Bug Fixes

- **English card images everywhere** — The banned/staple image resolvers localized TCGdex CDN URLs by request locale, breaking images for sets never printed in that language (blank Marshadow on `/fr/banned-cards`) and showing localized art under English card names (Flapple as Pomdrapi). Card images now always resolve to the English prints. ([#784](https://github.com/jbourdin/expandedDecks/pull/784))
- **Stray column drop in card-translation migration** — The F9.17 schema migration, diffed on a branch predating `Channel.draftLocales`, destroyed that column when replayed after F9.16's migration. ([#779](https://github.com/jbourdin/expandedDecks/pull/779))
- **TipTap editor crash** — The markdown editor intermittently threw « commandManager is null » under re-render-heavy parents: the extensions array was rebuilt every render, re-instantiating the editor. Extensions are memoized and destroyed-editor guards added. ([#771](https://github.com/jbourdin/expandedDecks/pull/771))
- **Translate entry points on every edit surface** — The Translate actions only existed on the page/archetype show views; they now appear everywhere a content can be edited from: the public archetype catalog, the admin lists and edit forms of all five content types, the intro-page actions on the card listings, and variant deck pages (linking into the archetype context view). All gated by translation rights. ([#786](https://github.com/jbourdin/expandedDecks/pull/786))
- **Field-level locale fallback** — A translation row with some fields still untranslated (FR archetype with a description but no name) rendered the empty field; localized getters now fall back **per field** to the English value (`Archetype` name/description/metaDescription, `Page` title, SEO resolvers treating blank strings as absent). ([#786](https://github.com/jbourdin/expandedDecks/pull/786))

### Security & Infrastructure

- **CSP nonces on inline scripts** — Every inline script carries a per-request nonce and the report-only policy declares it, clearing console CSP warnings. ([#771](https://github.com/jbourdin/expandedDecks/pull/771))
- **Dependency bumps** — PHP and JS dependency refresh from the Dependabot backlog (tiptap suite in exact-version lockstep, no security advisories left open). ([#765](https://github.com/jbourdin/expandedDecks/pull/765), [#766](https://github.com/jbourdin/expandedDecks/pull/766))
