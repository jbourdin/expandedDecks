...
- Short lists are padded with **face-down filler cards** (the Pokemon card back, committed at `public/images/card_back.jpg` — under `public/`, not `assets/`, because the production image prunes `assets/` after the frontend build) so the fan always spans the full spread width instead of floating narrow in the middle; fillers sit deepest at the left edge. If the asset is missing, fillers degrade to neutral placeholders.
