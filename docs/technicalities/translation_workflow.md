...
- **Queue (F9.12, F9.19):** one React component (`TranslationQueue`), tabs per content type (pages, archetypes, menu categories, banned cards, staple cards), contributor/reviewer flavors from `TranslationQueueProvider` (`GET /admin/translations/queue-data`). Variant work aggregates on the archetype row as counters — one row per context. When rows span more than one target language, a locale segmented control (All languages / per-locale) filters every tab; it stays hidden while only one locale has work.
